...
纯静态 HTML/CSS/JS，数据从真相源 JSON 动态渲染，无构建步骤。**所有图表内容 100% 数据驱动**：模型名、厂商、价格、坐标域刻度全部来自 JSON；改数据文件即改页面，无任何写死的价格或模型名。**页面标题与 meta 描述不含模型数量**（避免模型增删时改 title/meta，保持 SEO 稳定）；页面内可见的模型数由 JS 从数据读取渲染。
...
Gemini 3.1 Pro / Gemini 3.8 Flash / GPT 6 Astra / GPT 6 Sol / GPT 6 Luna / Claude Fable 5.1 / Claude Opus 5.5 / Claude Sonnet 5 / Grok 4.7 / Seed 2.1 / Qwen 3.8 Max / Qwen 3.7 Plus / Qwen 3.8 Flash / Hy4 preview / Deepseek V4 Pro / Deepseek V4.1 Flash / GLM 5.3 / GLM 5.3 Flash / Kimi K3 / MiMo 2.6 Pro / MiMo 2.6 Flash / Minimax M3

模型增删规则：同一产品线的版本更新代替旧版本（如 Gemini 3.7 Flash → 3.8 Flash、Claude Opus 5 → Opus 5.5、HY 3.0 → Hy4 preview）；只有全新产品线才追加（如 GPT 5.6 系列之外新增 GPT 6 Astra）。详见 `docs/update-protocol.md`。
